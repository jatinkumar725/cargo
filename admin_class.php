...
Class Action {
	...
	function save_parcel(){
		...
		foreach($price as $k => $v){
			...
			if(empty($id)){
				...
				while($i == 0){
                    ...
                    $result = $this->db->query("SELECT * FROM `parcels` order by id DESC limit 0,1");
                    $result = $result->fetch_array();
                    $resultId = $result['id'];
                     $randomNGen = $this->randomGeneratRef($resultId); 
                     $ref = "2022".$randomNGen; 
					//$ref = sprintf("%'012d",mt_rand(0, 999999999999));
                    //echo "SELECT * FROM parcels where reference_number = '$ref'"; exit;
             ...
             //echo "INSERT INTO parcels set $data"; exit;
      ...
      function randomGeneratRef($lastId){
             $strLngth = strlen((string) $lastId);
             $fnl =$lastId+1; 
             if($strLngth==1){
               $result = "00000".$fnl;
             }
	...
	function update_parcel(){
		//extract($_POST);
        if(!empty($_REQUEST['status-other'])){
          $result= "'".$_REQUEST['status-other']."'";
        }else{
        ...
        $id =  $_REQUEST['id'];
        //echo "UPDATE parcels set status= $result, location= $location where id = $id"; exit;
		$update = $this->db->query("UPDATE parcels set status= $result, location= $location where id = $id");
		$save = $this->db->query("INSERT INTO parcel_tracks set status= $result, location= $location , parcel_id = $id");
		if($update && $save)
			return 1;  
	}
